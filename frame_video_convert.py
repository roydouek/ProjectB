...
def image_seq_to_video(imgs_path, output_path='./video.mp4', fps=15.0):
    output = output_path
    img_array = []
    for filename,idx in enumerate(glob.glob(os.path.join(imgs_path, '*.jpg'))):
        print(os.path.join(imgs_path, f'{filename}.jpg'))
        img = cv2.imread(os.path.join(imgs_path, f'{filename}.jpg'))
